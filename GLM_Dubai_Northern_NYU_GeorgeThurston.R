...
png(paste0(output_folder,"Dubai_Northern_Clinics_patients_respiratory.jpg"),
...
###########################
# run this for injuries ###
###########################
# counts_season <- rbind(health_annual_mean[2:282, ],    # 2013
#                        health_annual_mean[2:322, ],    # 2014
#                        health_annual_mean[1:266, ])    # 2015
...
output_folder <- "D:/R_processing/plots_new/"
png(paste0(output_folder,"Seasonality_Dubai_Northern_Clinics_patients_respiratory.jpg"),
    width = 1700, height = 1050, units = "px", pointsize = 30,
    bg = "white", res = 150)
print(plot)
dev.off()
...
# remove lines with negative values in the detrend counts
...
output_folder <- "D:/R_processing/plots_new/"
png(paste0(output_folder,"NO_Seasonality_Dubai_Northern_Clinics_patients_respiratory.jpg"),
    width = 1700, height = 1050, units = "px", pointsize = 30,
    bg = "white", res = 150)
print(plot)
dev.off()
...
health_data$Hot.Humid[health_data$Temp > 35 & health_data$RH > 75] <- 1
...
model <- glm(detrend_counts ~ cb + ns(time,12)+ health_data$dow + ns(Temp, 6) +
            ns(Temp_3day, 6)+ ns(RH,3)+ ns(RH_3day, 3)+ health_data$Hot.Humid + health_data$holidays, 
             family=poisson(), health_data)
...
output_folder <- "D:/R_processing/plots_new/"
png(paste0(output_folder,"RR_glm_RESPIRATORY.jpg"),
    width = 1700, height = 1050, units = "px", pointsize = 30,
    bg = "white", res = 150)
print(plot)
dev.off()
...
for(i in 0:5) {
  ...
  mod <- glm(health_data$detrend_counts ~ PM25lag + ns(health_data$time,12) + health_data$holidays +
